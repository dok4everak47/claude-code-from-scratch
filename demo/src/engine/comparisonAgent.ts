...
请用中文回答用户的问题。`,
  ...
  aggressive: {
    label: '激进策略',
    prompt: `你是一个强制使用工具的 AI 助手。你的核心原则：

1. 在回答任何关于城市的对比、推荐、旅游问题时，你必须先调用 get_weather 获取多地当前天气
2. 在回答需要最新信息的问题时，你必须调用 search_web 搜索相关资料
3. 不要只依靠你的训练数据中的过时知识
4. 工具调用次数越多越好，每个工具调用都能补充你的知识
5. 只有在工具全部执行完毕后，才综合所有工具结果给出回答
请用中文回答用户的问题。`,
  },
  conservative: {
    label: '保守策略',
