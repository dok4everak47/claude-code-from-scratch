...
export const toolDefinitions: ToolDef[] = [
    ...
    },
    {
        name: "git_status",
        description:
            "Show git status of the current repository. Returns branch, staged/unstaged changes, and untracked files.",
        input_schema: {
            type: "object",
            properties: {},
            required: [],
            ...
            for (const file of files) {
                try {
                    ...
                    if (nameMatch && typeMatch) {
                        lines.push(
                            `- **[${nameMatch[1].trim()}](${file})** (${typeMatch[1].trim()}) — ${descMatch?.[1]?.trim() || ""}`,
                        );
                    }
                } catch {
                    /* skip */
...
export function checkPermission(
    ...
    if (mode === "plan") {
        if (EDIT_TOOLS.has(toolName)) {
            ...
            return {
                action: "deny",
                message: `Blocked in plan mode: ${toolName}`,
            };
        }
    ...
    if (needsConfirm) {
        ...
        if (mode === "dontAsk") {
            return {
                action: "deny",
                message: `Auto-denied (dontAsk mode): ${confirmMessage}`,
            };
        }
    ...
    switch (name) {
        case "count_lines": {
            ...
            try {
                ...
                if (stat.isDirectory()) {
                    // 递归统计目录下所有文件
                    function countDir(dir: string): { files: number; lines: number } {
                    ...
                    const r = countDir(targetPath);
                    result = `目录共 ${r.files} 个文件，${r.lines} 行代码`;
                } else {
                    const content = readFileSync(targetPath, "utf-8");
                    const lineCount = content.split("\n").length;
                    result = `文件共 ${lineCount} 行`;
                }
            } catch (e: any) {
                result = `错误: ${e.message}`;
            }
