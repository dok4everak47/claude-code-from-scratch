...
IMPORTANT: When your plan is complete, you MUST call exit_plan_mode. Do NOT ask the user to approve — exit_plan_mode handles that.`;
  ...
  private async chatOpenAI(userMessage: string): Promise<void> {
    ...
    while (true) {
      ...
      if (!this.isSubAgent) startSpinner();
      const earlyExecutions = new Map<string, Promise<string>>();
      const response = await this.callOpenAIStream((block) => {
        if (CONCURRENCY_SAFE_TOOLS.has(block.name)) {
          const perm = checkPermission(
            block.name,
            block.input,
          ...
          if (perm.action === "allow") {
            earlyExecutions.set(
              block.id,
              this.executeToolCall(block.name, block.input),
            );
          }
        }
      });
      ...
      for (const tc of toolCalls) {
        ...
        const earlyPromise = earlyExecutions.get(tc.id);
        if (earlyPromise) {
          const raw = await earlyPromise;
          const res = this.persistLargeResult(fnName, raw);
          printToolResult(fnName, res);
          this.openaiMessages.push({
            role: "tool",
            tool_call_id: tc.id,
            content: res,
      ...
      for await (const chunk of stream) {
        ...
        if (onToolBlockComplete && delta.tool_calls) {
          for (const tc of delta.tool_calls) {
            ...
            if (!entry.id || !entry.name || !entry.arguments) continue;
            try {
