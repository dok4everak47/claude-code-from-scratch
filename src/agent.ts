...
export class Agent {
    ...
    constructor(options: AgentOptions = {}) {
        ...
        this.baseSystemPrompt =
            options.customSystemPrompt || buildSystemPrompt();
        if (this.permissionMode === "plan") {
            this.planFilePath = this.generatePlanFilePath();
            this.systemPrompt =
                this.baseSystemPrompt + this.buildPlanModePrompt();
        } else {
    ...
    showCost() {
        ...
        const budgetInfo = this.maxCostUsd
            ? ` / $${this.maxCostUsd} budget`
            : "";
        const turnInfo = this.maxTurns
    ...
    private checkBudget(): { exceeded: boolean; reason?: string } {
        if (
            ...
            this.getCurrentCostUsd() >= this.maxCostUsd
        ) {
            return {
                exceeded: true,
                reason: `Cost limit reached ($${this.getCurrentCostUsd().toFixed(4)} >= $${this.maxCostUsd})`,
            };
        }
        if (this.maxTurns !== undefined && this.currentTurns >= this.maxTurns) {
            return {
                exceeded: true,
                reason: `Turn limit reached (${this.currentTurns} >= ${this.maxTurns})`,
            };
        }
    ...
    restoreSession(data: {
        ...
        if (data.anthropicMessages)
            this.anthropicMessages = data.anthropicMessages;
    ...
    private autoSave() {
        try {
            saveSession(this.sessionId, {
                ...
                anthropicMessages: this.useOpenAI
                    ? undefined
                    : this.anthropicMessages,
                openaiMessages: this.useOpenAI
                    ? this.openaiMessages
                    : undefined,
            });
        } catch {}
    ...
    private snipStaleResultsAnthropic(): void {
        ...
        for (const idx of toSnip) {
            const r = results[idx];
            const block = (this.anthropicMessages[r.msgIdx].content as any[])[
                r.blockIdx
            ];
            block.content = SNIP_PLACEHOLDER;
        }
    ...
    private snipStaleResultsOpenAI(): void {
        ...
        for (let i = 0; i < snipCount; i++) {
            (this.openaiMessages[toolMsgs[i].idx] as any).content =
                SNIP_PLACEHOLDER;
        }
    }
    ...
    private microcompactAnthropic(): void {
        ...
        for (let i = 0; i < clearCount && i < allResults.length; i++) {
            ...
            (this.anthropicMessages[r.msgIdx].content as any[])[
                r.blockIdx
    ...
    private microcompactOpenAI(): void {
        ...
        for (let i = 0; i < clearCount && i < toolMsgs.length; i++) {
            (this.openaiMessages[toolMsgs[i]] as any).content =
                "[Old result cleared]";
        }
    }
    ...
    private async executeSkillTool(
        ...
        if (result.context === "fork") {
            ...
            const subAgent = new Agent({
                model: this.model,
                apiBase: this.useOpenAI
                    ? this.openaiClient?.baseURL
                    : undefined,
            ...
            try {
                const subResult = await subAgent.runOnce(
                    input.args || "Execute this skill task.",
                );
...
IMPORTANT: When your plan is complete, you MUST call exit_plan_mode. Do NOT ask the user to approve — exit_plan_mode handles that.`;
    ...
    private async executePlanModeTool(name: string): Promise<string> {
        if (name === "enter_plan_mode") {
            ...
            printInfo(
                "Entered plan mode (read-only). Plan file: " +
                    this.planFilePath,
            );
        ...
        if (name === "exit_plan_mode") {
            ...
            if (this.planApprovalFn) {
                ...
                if (result.choice === "keep-planning") {
                    ...
                    const feedback =
                        result.feedback || "Please revise the plan.";
    ...
    private async chatAnthropic(userMessage: string): Promise<void> {
        ...
        while (true) {
            ...
            if (
                memoryPrefetch &&
                memoryPrefetch.settled &&
                !memoryPrefetch.consumed
            ) {
                ...
                try {
                    ...
                    if (memories.length > 0) {
                        ...
                        const last =
                            this.anthropicMessages[
                                this.anthropicMessages.length - 1
                            ];
                        ...
                        for (const m of memories) {
                            ...
                            this.sessionMemoryBytes += Buffer.byteLength(
                                m.content,
                            );
                        }
                    }
            ...
            const response = await this.callAnthropicStream((block) => {
                ...
                if (CONCURRENCY_SAFE_TOOLS.has(block.name)) {
                    ...
                    if (perm.action === "allow") {
                        earlyExecutions.set(
                            block.id,
                            this.executeToolCall(block.name, input),
                        );
                    }
                }
            });
    ...
    private async callAnthropicStream(
        ...
        return withRetry(async (signal) => {
            ...
            const createParams: any = {
                model: this.model,
                max_tokens:
                    this.thinkingMode !== "disabled" ? maxOutput : 16384,
            ...
            const stream = this.anthropicClient!.messages.stream(createParams, {
                signal,
            });
    ...
    private async chatOpenAI(userMessage: string): Promise<void> {
        ...
        while (true) {
                ...
                try {
                    ...
                    if (memories.length > 0) {
                        const injectionText =
                            formatMemoriesForInjection(memories);
                        const last =
                            this.openaiMessages[this.openaiMessages.length - 1];
                        if (last && last.role === "user") {
                            last.content =
                                (last.content || "") + "\n\n" + injectionText;
                        } else {
            ...
            for (const tc of toolCalls) {
                ...
                if (perm.action === "deny") {
                    ...
                    oaiChecked.push({
                        ...
                        input,
                        allowed: false,
                        result: `Action denied: ${perm.message}`,
                    });
                    continue;
                }
                if (
            ...
            for (const ct of oaiChecked) {
                const safe =
                    ct.allowed && CONCURRENCY_SAFE_TOOLS.has(ct.fnName);
                if (
                    safe &&
                    oaiBatches.length > 0 &&
                    oaiBatches[oaiBatches.length - 1].concurrent
                ) {
            ...
            for (const batch of oaiBatches) {
                if (oaiContextBreak || this.abortController?.signal.aborted)
                    break;
    ...
    private async callOpenAIStream(
        ...
        return withRetry(async (signal) => {
            ...
            for await (const chunk of stream) {
                ...
                if (onToolBlockComplete && delta.tool_calls) {
                    for (const tc of delta.tool_calls) {
