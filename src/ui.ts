...
export function printWelcome() {
    ...
    console.log(
        chalk.gray("  Commands: /clear /plan /cost /compact /memory /skills\n"),
    );
}
...
function printFileChangeResult(name: string, result: string) {
    ...
    if (contentLines.length > maxDisplayLines) {
        console.log(
            chalk.gray(
                `  ... (${contentLines.length - maxDisplayLines} more lines)`,
            ),
        );
    }
}
...
export function startSpinner(label = "Thinking") {
    ...
    spinnerTimer = setInterval(() => {
        ...
        process.stdout.write(
            `\r${chalk.gray(`  ${SPINNER_FRAMES[spinnerFrame]} ${label}...`)}`,
        );
    }, 80);
}
...
export function printPlanApprovalOptions() {
    ...
    console.log(
        chalk.white("    1) Yes, clear context and execute") +
            chalk.gray(" — fresh start with auto-accept edits"),
    );
    console.log(
        chalk.white("    2) Yes, and execute") +
            chalk.gray(" — keep context, auto-accept edits"),
    );
    console.log(
        chalk.white("    3) Yes, manually approve edits") +
            chalk.gray(" — keep context, confirm each edit"),
    );
    console.log(
        chalk.white("    4) No, keep planning") +
            chalk.gray(" — provide feedback to revise"),
    );
}
